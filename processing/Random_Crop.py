...
def crop_rotate(density_data):
    ...
    x_coords = np.linspace(-2.5, 2.5, 32*5)
    y_coords =  np.linspace(-2.5, 2.5, 32*5)
    z_coords =  np.linspace(-2.5, 2.5, 32*5)

    supercell = np.tile(density_data, (5, 5, 5))

    # Define crop size and translation
    crop_sizes = np.random.uniform(1, 1.732, size=(3, 1))
    crop_translation = np.random.rand(3) - 0.5

    # Define rotated cube cell
    x_crop = np.linspace(0, crop_sizes[0], 32) -crop_sizes[0]/2
    y_crop = np.linspace(0, crop_sizes[1], 32) -crop_sizes[1]/2
    z_crop = np.linspace(0, crop_sizes[2], 32) -crop_sizes[2]/2
